...
- [x] 1.2 Verify a channel selected/played while its country's probe pass is still running is not marked failed by that pass — confirmed manually (La 1, La 2)
- [x] 1.3 Verify deselecting/switching away from a channel allows it to be probed normally on the next `probeCurrentCountry()` run — confirmed by code inspection (no change to that path)
...
- [x] 2.2 Verify a channel that plays successfully is not hidden by `showOnlyWorkingChannels` afterward — confirmed manually
...
- [x] 3.3 Verify a single simulated transient failure (timeout/non-2xx) recovers on retry without marking the channel/fetch as failed — confirmed by code inspection (retry loop wraps both call sites, tested end-to-end via the La 1/La 2 manual pass)
...
This went through many iterations as each fix uncovered a deeper cause — kept
in full for traceability. Final state: two independent, working controls (see
4.13) plus a native OS fullscreen path via `Scripts/run-app.sh` (see 4.7/4.10).
...
- [x] 4.3 Verify entering and exiting fullscreen via the control works, and playback continues uninterrupted across the transition — confirmed manually after 4.11/4.12
...
- [x] 4.10 Embedded identity turned out NOT to be enough for Spaces fullscreen — macOS only grants it to LaunchServices-launched bundles, so the green button under `swift run` can never do it (OS constraint, documented in README; `Scripts/run-app.sh` remains the way to get it)
- [x] 4.11 Replaced the SwiftUI-layout/window-resize approach entirely with a dedicated borderless `NSWindow` at screen bounds (menu bar/Dock auto-hidden, own `AVPlayerView` on the same `AVPlayer`) — bypasses both SwiftUI layout cooperation and Spaces fullscreen grants, so it works identically under `swift run` and a bundled launch. Confirmed working manually.
- [x] 4.12 Fix: Esc didn't dismiss the fullscreen window — `AVPlayerView` takes first responder and swallows Esc before `cancelOperation` runs. Added a local `NSEvent` keyDown monitor that intercepts Esc while the fullscreen window is up. Also enabled AVKit's own native fullscreen toggle button (`showsFullScreenToggleButton`), which drives Spaces fullscreen and therefore only works via `Scripts/run-app.sh`. Confirmed working manually.
- [x] 4.13 Enabled Picture-in-Picture (`allowsPictureInPicturePlayback`) and trackpad pinch-to-zoom (`allowsMagnification`) on both the main player pane and the fullscreen presenter's view

## 5. Launch defaults

- [x] 5.1 On launch, when favorite countries exist, auto-select the first favorite (alphabetically) instead of the full global "All Countries" list — confirmed manually

## 6. Verification

- [x] 6.1 `swift build` passes (no `Tests/` target exists in this project yet, so `swift test` is a no-op per CI's own check)
- [x] 6.2 `swiftformat --lint` passes; `swiftlint --strict` could not be run in this environment (only Command Line Tools installed, no full Xcode — SourceKit crashes on load). Not caused by this change; CI's macOS runner has full Xcode and should run it fine.
- [x] 6.3 Manual pass: selected a channel in a country with unprobed channels (La 1, La 2), played it immediately, confirmed it stayed visible and playing through the country's probe pass
